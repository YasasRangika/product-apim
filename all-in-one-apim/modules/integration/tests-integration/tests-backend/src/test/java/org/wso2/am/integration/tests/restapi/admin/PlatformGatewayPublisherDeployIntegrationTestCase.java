...
public class PlatformGatewayPublisherDeployIntegrationTestCase extends APIMIntegrationBaseTest {
    ...
    public void testPublisherDeployToUniversalSyncsInternalDeploymentsAndFetchBatch() throws Exception {
        ...
        try {
            ...
            byte[] rawBatch = PlatformGatewayTestUtils.postFetchBatchRaw(token, deploymentId);
    ...
    public void testFetchBatchTarContainsPlatformGatewayYamlMarker() throws Exception {
        ...
        try {
            ...
            Assert.assertTrue(PlatformGatewayTestUtils.containsAscii(tarBytes, "ustar"),
                    "fetch-batch should return a tar archive payload after gzip unwrap");
            String searchable = PlatformGatewayTestUtils.tarPayloadAsSearchableText(tarBytes);
            Assert.assertTrue(searchable.contains(PLATFORM_GATEWAY_YAML_API_VERSION),
